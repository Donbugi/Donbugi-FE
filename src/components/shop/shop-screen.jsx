...
const earnItems = [
  { icon: '⚔️', title: '오늘의 과제 참여', desc: '문제 풀기만 해도 · 문제당', points: '+20P', tag: '문제당' },
  { icon: '🎯', title: '오늘의 과제 전체 정답', desc: '3문제 모두 정답 시 보너스', points: '+20P', tag: '보너스' },
  { icon: '📰', title: '뉴스 5개 읽기', desc: '뉴스 상세 페이지 5개 이상 방문', points: '+20P', tag: '달성시' },
  { icon: '📖', title: '뉴스 10개 읽기', desc: '뉴스 상세 페이지 10개 이상 방문', points: '+40P', tag: '달성시' },
  { icon: '🧩', title: '뉴스 상세 퀴즈 참여', desc: '뉴스 상세 페이지 퀴즈 참여', points: '+20P', tag: '참여당' },
  { icon: '🔥', title: '연속 7일 출석 보너스', desc: '7일 연속 출석 달성 시', points: '+100P', tag: '보너스' },
]

const exchangeItems = [
  { icon: '💳', name: '간편결제 캐시백', desc: '카카오페이·토스 500원 캐시백', cost: '1,000P' },
  { icon: '☕', name: '커피 쿠폰', desc: '스타벅스·메가커피 아메리카노 1잔', cost: '800P' },
]

const exchangeGridItems = [
  { icon: '📡', name: '데이터 쿠폰', desc: '통신사 1GB 충전', cost: '1,500P' },
  { icon: '🏪', name: '편의점 할인', desc: 'GS25·CU 1,000원 할인', cost: '600P' },
    ...
    <div className="flex-1 overflow-y-auto hide-scrollbar">
      ...
      <div className="bg-gradient-to-br from-[#0f0c29] via-[#302b63] to-[#24243e] text-white relative overflow-hidden">
        ...
        <div className="px-5 pt-6 pb-5 relative z-10">
          ...
          <p className="text-[13px] opacity-75">활동으로 포인트를 모아 혜택으로 교환하세요</p>
          ...
          <div className="bg-white/[0.08] border border-white/[0.12] rounded-[16px] p-4 mt-4 flex items-center justify-between backdrop-blur-[16px]">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-[11px] opacity-70 font-bold tracking-[0.5px]">보유 포인트</div>
                <div className="text-[26px] font-black text-[#ffd700] leading-[1.1]">3,240P</div>
                <div className="text-[11px] text-white/50 mt-[2px]">이번 달 +1,240P 적립</div>
              </div>
            </div>
            <div className="bg-[rgba(60,187,162,0.2)] border border-[rgba(60,187,162,0.4)] rounded-[10px] py-1.5 px-3 text-center">
              <div className="text-sm font-black text-[#3CBBA2]">{char.lvLabel}</div>
      ...
      <div className="p-4 pb-6">
        {activeTab === 'earn' ? (
          <>
            <div className="mb-3">
              <div className="text-[15px] font-black text-[#1a1a2e]">포인트 적립 방법</div>
              <div className="text-[12px] text-[#8888aa] mt-[2px]">활동에 참여하고 포인트를 적립하세요</div>
            </div>
              ...
              {earnItems.map((item, i) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="text-sm font-black text-[#1a1a2e]">{item.title}</div>
                    <div className="text-[12px] text-[#8888aa] mt-[2px]">{item.desc}</div>
                  </div>
                  <div className="flex flex-col items-end gap-[2px]">
                    <div className="text-sm font-black text-[#3CBBA2]">{item.points}</div>
          ...
          <>
            <div className="text-[13px] font-black text-[#8888aa] tracking-[0.5px] mt-4 mb-2">인기 혜택</div>
            ...
            {exchangeItems.map((item, i) => (
              ...
              >
                <div className="flex items-center gap-3 mb-2.5">
                  ...
                  <div className="flex-1">
                    <div className="text-[15px] font-black text-[#1a1a2e]">{item.name}</div>
                    <div className="text-[12px] text-[#8888aa] leading-[1.4] mt-[2px]">{item.desc}</div>
                  </div>
                </div>
                <div className="flex items-center justify-between pt-2.5 border-t border-[rgba(0,0,0,0.05)]">
                  <div>
                    <div className="text-[13px] font-black text-[#7C3AED]">{item.cost}</div>
                    <div className="text-[11px] text-[#8888aa]">필요 포인트</div>
                  </div>
            ...
            <div className="text-[13px] font-black text-[#8888aa] tracking-[0.5px] mt-4 mb-2">생활 혜택</div>
            ...
            <div className="grid grid-cols-2 gap-2.5">
              {exchangeGridItems.map((item, i) => (
                ...
                >
                  ...
                  <div className="text-[13px] font-black text-[#1a1a2e]">{item.name}</div>
                  <div className="text-[11px] text-[#8888aa] my-1 mb-2">{item.desc}</div>
                  <div className="flex items-center justify-between pt-2 border-t border-[rgba(0,0,0,0.05)]">
                    <div className="text-[12px] font-black text-[#7C3AED]">{item.cost}</div>
            ...
            <div className="bg-[rgba(124,58,237,0.05)] rounded-xl py-3 px-3.5 text-[12px] text-[#8888aa] leading-[1.7] mt-3.5 border border-[rgba(124,58,237,0.08)]">
              💡 교환 신청 시 가입 이메일로 쿠폰이 자동 발송됩니다. 발송까지 최대 24시간이 소요될 수 있어요.
            </div>
          </>
        )}
      </div>
